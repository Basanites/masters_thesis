...
impl<Nw: Clone, Ew: Clone> GenericWeightedGraph<usize, Nw, Ew> for MatrixGraph<Nw, Ew> {
    ...
    fn iter_node_ids(&self) -> Box<dyn Iterator<Item = usize> + '_> {
        Box::new(self.node_weights.iter()
            .enumerate()
    ...
    fn iter_neighbor_ids(&self, id: usize) -> Result<Box<dyn Iterator<Item = usize> + '_>, GraphError> {
        if !self.has_node(id) {
            return Err(GraphError::MissingNode(id))
        }

        // Get the ids of nodes to which a weighted edge from id exists.
        Ok(Box::new(self.adjacency_matrix[id].iter()
               .enumerate()
               .filter(|(_, x)| x.is_some())
               .map(move |(i, _)| i)))
    }
    ...
    fn iter_neighbors(&self, id: usize) -> Result<Box<dyn Iterator<Item = (usize, &Ew)> + '_>, GraphError> {
        if !self.has_node(id) {
            return Err(GraphError::MissingNode(id))
        }
    ...
    fn iter_edge_ids(&self) -> Box<dyn Iterator<Item = Edge<usize>> + '_> {
        Box::new(self.adjacency_matrix.iter()
            .enumerate()
            .flat_map(|(i, edges)| {
                edges.iter()
                    .enumerate()
...
mod tests {
    ...
    fn valid_weighted() -> MatrixGraph<usize, usize> {
        MatrixGraph::new(vec![1, 2, 3], vec![(0, 1, 100), (1, 2, 101), (2, 1, 50), (2, 0, 200)]).unwrap()
    }
    ...
    fn new_empty_weighted_works() {
        ...
        assert_eq!(graph.node_weights, Vec::new(), "Node weight array is not empty.");
        assert_eq!(graph.adjacency_matrix, Vec::<Vec<Option<usize>>>::new(), "Adjacency matrix is not empty.");
    ...
    fn new_weighted_with_size_works() {
        ...
        assert!(graph.node_weights.capacity() >= 5, "Not enough space given for nodes.");
        for weight in graph.node_weights.iter() {
            assert!(weight.is_none(), "Node weight was initialized with a value.");
        }
    ...
    fn new_weighted_from_lists_works() {
        ...
        assert!(graph.node_weights.capacity() >= 3, "Array was created too small.");
        assert_eq!(graph.node_weights, vec![Some(1), Some(2), Some(3)], "Node weights are wrong.");
        assert_eq!(graph.adjacency_matrix, vec![
    ...
    fn new_vector_weighted_from_lists_works() {
        ...
        assert!(graph.node_weights.capacity() >= 3, "Array is to small to hold all nodes.");
        assert_eq!(graph.node_weights, vec![Some(vec![1, 2, 3]), Some(vec![4, 5, 6]), Some(vec![7, 8, 9])], "Node weights are wrong.");
        assert_eq!(graph.adjacency_matrix, vec![
    ...
    fn new_with_missing_from_node_errors() {
        ...
        assert_eq!(err, Some(GraphError::MissingNode(1)), "Not missing the node it should be missing.");
    }
    ...
    fn order_works() {
        ...
        assert_eq!(not_empty.order(), 3, "Graph should have three nodes, thus order 3.");
        assert_eq!(empty.order(), 0, "Graph should be empty, thus have order 0.");
    }
    ...
    fn size_works() {
        ...
        assert_eq!(not_empty.size(), 4, "Graph should have four edges, thus size 4.");
    ...
    fn nodes_works() {
        ...
        assert_eq!(graph.nodes(), vec![0, 1, 2, 4], "The node 4 should be in, since it was just added.");
    }
    ...
    fn node_weight_errors_for_missing() {
        ...
        assert_eq!(err, Some(GraphError::MissingNode(5)), "The node 5 should not be in the graph");
    }
    ...
    fn neighbors_errors_for_missing() {
        let graph = valid_weighted();
        let err = graph.neighbors(5).err();

        assert_eq!(err, Some(GraphError::MissingNode(5)), "The node 5 should not be in the graph, thus has no neighbors.");
    }
    ...
    fn adding_nodes_works() {
        ...
        assert_eq!(graph.order(), 4, "Order was not updated correctly after insertion.");
        assert_eq!(graph.size(), 4, "Size should not change during node insertion.");
        assert_eq!(graph.nodes().sort(), vec![0, 1, 2, 4].sort(), "Node list was not updated correctly after insertion.");
        assert_eq!(graph.node_weight(4).unwrap(), &5, "Incorrect weight was applied to new node.");
        assert!(graph.node_weights.len() >= 4, "Not enough space for all possible node weights.");
        assert!(graph.adjacency_matrix.len() >= 4, "Not enough space for all possible edge weights.");
        for edges in graph.adjacency_matrix.iter() {
            assert!(edges.len() >= 4, "Not enough space for all possible edge weights.");
        }
    }
    ...
    fn adding_duplicate_node_errors() {
        ...
        assert_eq!(err, Some(GraphError::DuplicateNode(1)), "The node should be a duplicate.");
    }
    ...
    fn removing_nodes_works() {
        ...
        assert_eq!(graph.order(), 2, "Order was updated incorrectly after removal.");
        assert_eq!(graph.size(), 1, "Size was updated incorrectly after removal.");
        assert_eq!(graph.nodes().sort(), vec![0, 2].sort(), "Nodelist was updated incorrectly after removal.");
        assert_eq!(graph.edge_ids().sort(), vec![(2, 0)].sort(), "Edgelist was updated incorrectly after removal.");
    }
    ...
    fn degree_works() {
        ...
        assert_eq!(graph.degree(0).unwrap(), 1, "Node 0 only has an edge to node 1.");
        assert_eq!(graph.degree(2).unwrap(), 2, "Node 2 has an edge to node 0 and node 1.");
    }
    ...
    fn degree_errors_for_missing() {
        ...
        assert_eq!(err, Some(GraphError::MissingNode(5)), "Graph does not have node 5.");
    }
    ...
    fn edges_works() {
        ...
        assert_eq!(graph.edge_ids(), vec![(0,1), (1, 2), (2, 0), (2, 1)], "Edge list is wrong.");
        assert_eq!(empty.edge_ids(), vec![], "Edge list should be empty, since graph is empty.");
    }
    ...
    fn edge_weight_works() {
        ...
        assert_eq!(graph.edge_weight((0, 1)).unwrap(), &100, "Edge has wrong weight.");
    }
    ...
    fn edge_weight_errors_for_missing() {
        ...
        assert_eq!(graph.edge_weight((1, 1)).err(), Some(GraphError::MissingEdge((1, 1))), "Edge (1, 1) should not be in the graph.");
    }
    ...
    fn has_edge_works() {
        ...
        assert!(graph.has_edge((0, 1)), "Graph should have an edge from node 0 to node 1.");
        assert!(graph.has_edge((2, 0)), "Graph should have an edge from node 2 to node 0.");
        assert!(!graph.has_edge((1, 0)), "Graph should not have an edge from node 1 to node 0.");
    }
    ...
    fn adding_edge_works() {
        ...
        assert_eq!(graph.edge_weight((1, 0)).unwrap(), &17, "Edge has wrong weight.")
    }
    ...
    fn adding_duplicate_edge_errors() {
        ...
        assert_eq!(err, Some(GraphError::DuplicateEdge((0, 1))), "The edge should be a duplicate.")
    }
    ...
    fn adding_edge_with_missing_node_errors() {
        ...
        assert_eq!(err1, Some(GraphError::MissingNode(5)), "The node 5 should not be in the graph.");
        assert_eq!(err2, Some(GraphError::MissingNode(5)), "The node 5 should not be in the graph.");
    }
    ...
    fn removing_edge_works() {
        ...
        graph.remove_edge((0, 1));
        assert!(!graph.has_edge((0, 1)), "Multiple deletions insert the edge back.");
    }
