...
/// calculates the distance between two points in m assuming point.x is its lat and point.y is its lon in degrees
/// done using formula from https://en.wikipedia.org/wiki/Haversine_formula
fn geodistance_haversine(point_a: Point, point_b: Point) -> f64 {
    let lat_a = degrees_to_radians(point_a.x);
    let lon_a = degrees_to_radians(point_a.y);
    let lat_b = degrees_to_radians(point_b.x);
    let lon_b = degrees_to_radians(point_b.y);
    // average earth radius is assumed to be 6371km
    2.0 * 6371.0 * (((lat_b - lat_a) / 2.0).sin().powi(2) + lat_a.cos() * lat_b.cos() * ((lon_b - lon_a) / 2.0).sin().powi(2)).sqrt().asin()
}

/// Calculates the traveltime in minutes for a given distance_map
...
fn get_node_replacement (replacement_map: &HashMap<OsmId, (OsmId, HashMap<String, f64>)>, replacement: OsmId, dist_map: HashMap<String, f64>) -> (OsmId, HashMap<String, f64>) {
...
fn find_contractable_nodes(neighbors: &HashMap<OsmId, HashMap<OsmId, HashMap<String, f64>>>,
...
fn get_node_distance(node_1: &Node, node_2: &Node) -> f64 {
    ...
    return geodistance_haversine(p1, p2);
}
...
fn main() -> std::io::Result<()> {
    ...
    for edge in mapped_graph.edges().iter() {
       println!("Edge {:?} takes {:?} minutes", edge, mapped_graph.edge_weight(*edge));
    }
    ...
    println!("distance between {:?} and {:?} is {:?}km", p1, p2, geodistance_haversine(p1, p2));
